...
- `Microsoft Visual Studio <https://www.visualstudio.com/>`__: A free, rich, IDE that now natively supports Python, and Anaconda distribution. CPython, IronPython, the IPython REPL, Debugging, Profiling, Git, GitHub, are also supported. Also has a lighter version, named Code, which is a code editor with debugger. Both feature powerful IntelliSense, and support Windows and macOS, plus Linux in case of Code.
