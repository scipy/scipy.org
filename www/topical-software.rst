...
- `Microsoft Visual Studio <https://www.visualstudio.com/>`__: A free, rich IDE that now natively supports Python and Anaconda. CPython, IronPython, the IPython REPL, debugging, profiling, Git and GitHub are also supported. Also has a lighter version, named Visual Studio Code, which is a code editor with debugger. Both feature powerful IntelliSense, and support Windows and OS X, plus Linux in case of Code.
