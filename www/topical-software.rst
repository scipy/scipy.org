...
- `Plotly <https://plot.ly/>`__: A collaborative browser-based plotting and analytics platform. You can generate graphs and analyze data from the in-browser Python sandbox (numpy supported) or the `Plotly API <https://plot.ly>`__. Plotly supports `interactive IPython graphs <http://nbviewer.ipython.org/7105191>`__. Graphs are interactive, can be styled with Python or the GUI, and are sharable by link. Scripts, graphs, and data can be shared, collaboratively edited, and stored and run in Plotly.
